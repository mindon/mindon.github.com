...
This code may only be used under the BSD style license found at https://polymer.github.io/LICENSE
The complete set of authors may be found at https://polymer.github.io/AUTHORS
The complete set of contributors may be found at https://polymer.github.io/CONTRIBUTORS
Code distributed by Google as part of the polymer project is also
subject to an additional IP rights grant found at https://polymer.github.io/PATENTS
-->
<html>
<head>
  <meta charset="UTF-8">
...
function scrollTestHelper(scroller, tests) {
  ...
  function triggerScrollEvent() {
    var scrollTop = scroller === document.documentElement ? window.pageYOffset :
                                                            scroller.scrollTop;
    ...
    flush(function() {
      scrollTestHelper(container.scrollTarget, [
        ...
        },
        {
          ...
          callback: function() {
            assert.deepEqual(
                testEffect.run.lastCall.args,
                [0.5, container.offsetHeight * 0.5]);
          }
        },
        {
          ...
          callback: function() {
            assert.deepEqual(
                testEffect.run.lastCall.args, [1, container.offsetHeight]);
          }
        },
        {
          ...
          callback: function() {
            assert.deepEqual(
                testEffect.run.lastCall.args, [2, container.offsetHeight * 2]);
          }
        },
        {
          ...
          callback: function() {
            assert.deepEqual(
                testEffect.run.lastCall.args, [3, container.offsetHeight * 3]);
            container.effects = '';
      ...
      scrollTestHelper(container.scrollTarget, [
        {
          ...
          callback: function() {
            assert.isTrue(testEffect.setUp.called);
            assert.deepEqual(
                testEffect.setUp.lastCall.args, [testEffectConfig]);
            assert.deepEqual(testEffect.run.lastCall.args, [0, 0]);
          }
        },
        {
        ...
        },
        {
          ...
          callback: function() {
            assert.deepEqual(
                testEffect.run.lastCall.args,
                [0.5, container.offsetHeight * 0.75]);
          }
        },
        {
          y: container.offsetHeight,
