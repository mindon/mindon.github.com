  ...
  suite('events across shadow boundaries', function() {
    ...
    suite('descendant\'s iron-resize', function() {
      test('only fires once for a notifying shadow root', function() {
        var childResizable1InitialCount =
            resizable.$.childResizable1.ironResizeCount;
        var childResizable2InitialCount =
            resizable.$.childResizable2.ironResizeCount;

        resizable.notifyResize();

        expect(
            resizable.$.childResizable1.ironResizeCount -
            childResizable1InitialCount)
            .to.be.equal(1);
        expect(
            resizable.$.childResizable2.ironResizeCount -
            childResizable2InitialCount)
            .to.be.equal(1);
      });
      ...
      test('only fires once for a notifying descendent observable', function() {
        ...
        expect(resizable.$.childResizable1.ironResizeCount - initialCount)
            .to.be.equal(1);
      });
    });
